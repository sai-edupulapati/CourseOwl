...
rootEngineering = "https://engineering.purdue.edu/Engr/People/ptDirectory?letter="
linkEngineeering = "https://engineering.purdue.edu/Engr/People/ptProfile?resource_id="
imageBaseEngineering = "https://engineering.purdue.edu/ResourceDB/ResourceFiles/image"


def storingIntoCsv(root) :
    info = gettingAllTeachersInfo(root)
    with open('professorEngineeringInfo.csv', 'w', newline='') as file:
...
def gettingInfoFromEachTeacherLink(singleTeacherLink) :
    ...
    try :
        imageLink = [x['src'] for x in allImages if imageBaseEngineering in x['src']][0]
    except :
        imageLink = ""
...
def getLinksFromPage(eachAlphabetLink):
    ...
    linksNeeded = [x['href'] for x in allLinks if linkEngineeering in x['href']]
    uniqueLinks = set(linksNeeded)
    return uniqueLinks

storingIntoCsv(rootEngineering)